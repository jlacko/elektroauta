...
ggplot(reg_src, aes(x = total_net_income, y = pct_friendly)) +
  ...
  labs(title = "Registrace elektro a hybridních aut podle výše příjmů domácností",
       x = "Průměrný příjem domácnosti",
...
ggplot(reg_src, aes(x = vysoky_prijem, y = pct_friendly)) +
  ...
  labs(title = "Registrace elektro a hybridních aut podle podílu vysokopříjmových domácností",
       x = "Podíl domácností v rozmezí 30-50 tisíc Kč příjmu",
       y = "Podíl elektro a hybridních aut") +
  scale_y_continuous(labels = scales::percent,
                     breaks = seq(0, .0175, length.out = 5),
                     limits = c(0, .0175)) +
...
ggplot(reg_src, aes(x = detached_house, y = pct_friendly)) +
  geom_point(pch = 4) +
  geom_smooth(method = "lm") +
  labs(title = "Registrace elektro a hybridních aut podle podílu domácností v rodinném domě",
       x = "Podíl domácností v rodinném domě",
       y = "Podíl elektro a hybridních aut") +
  scale_y_continuous(labels = scales::percent,
                     breaks = seq(0, .0175, length.out = 5),
                     limits = c(0, .0175)) +
